...
with open('./classification_model/metadata/city_coords.json', 'r') as f:
...
def get_coordinates_from_city_str(df: pd.DataFrame, column: str = 'city') -> pd.DataFrame:
...
def preprocess_dataframe(df: pd.DataFrame, test_mode: bool = False) -> pd.DataFrame:
    ...
    df = get_coordinates_from_city_str(df)
    print('----')
